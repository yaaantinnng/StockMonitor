...
def plot_individual_k_lines(symbol):
    ...
    html_file_content = f"<h2>{symbol} K-Line Charts</h2>"
    # 日K线图
    fig_daily = go.Figure()
    ...
    fig_daily.update_layout(
        title=f"{symbol} Daily K-line (Recent 15 Days)",
        xaxis_title="Date", yaxis_title="Price (CNY)", template="plotly_dark",
        xaxis=dict(type='category'),  # 将x轴设置为类别型
        xaxis_rangeslider_visible=False
    )
    html_file_content += fig_daily.to_html(full_html=False, include_plotlyjs='cdn')
    ...
    fig_weekly.update_layout(
        title=f"{symbol} Weekly K-line (Recent 15 Weeks)",
        xaxis_title="Date", yaxis_title="Price (CNY)", template="plotly_dark",
        xaxis=dict(type='category'),  # 将x轴设置为类别型
        xaxis_rangeslider_visible=False
    )
    html_file_content += fig_weekly.to_html(full_html=False, include_plotlyjs='cdn')
    ...
    fig_monthly.update_layout(
        title=f"{symbol} Monthly K-line (All History)",
        xaxis_title="Date", yaxis_title="Price (CNY)", template="plotly_dark",
        xaxis=dict(type='category'),  # 将x轴设置为类别型
        xaxis_rangeslider_visible=False
    )
    ...
    html_file_content += fig_monthly.to_html(full_html=False, include_plotlyjs='cdn')
